...
fn main() {
    ...
    fn starter_swap(data: &[u8]) -> u64 {
        ...
        let k = rx * ry;
        match side {
            0 => { let net = input * 950 / 1000; let new_ry = ry + net; rx.saturating_sub((k + new_ry - 1) / new_ry) as u64 }
            1 => { let net = input * 950 / 1000; let new_rx = rx + net; ry.saturating_sub((k + new_rx - 1) / new_rx) as u64 }
            _ => 0,
        }
    }
