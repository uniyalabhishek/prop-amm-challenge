...
fn exp_swap_with_params(data: &[u8], fee_bps: f64, alpha: f64) -> u64 {
...
// Storage-fee version for run_sim
fn storage_exp_swap(data: &[u8]) -> u64 {
    if data.len() < 29 { return exp_swap_with_params(data, 55.0, 0.30); }
    let fee_bps = u16::from_le_bytes([data[25], data[26]]) as f64;
    let alpha_pct = u16::from_le_bytes([data[27], data[28]]) as f64;
    let fee = if fee_bps == 0.0 { 55.0 } else { fee_bps };
    let alpha = if alpha_pct == 0.0 { 0.30 } else { alpha_pct / 100.0 };
    exp_swap_with_params(data, fee, alpha)
}

fn run_sim(config: &SimulationConfig, fee_bps: u16, alpha_pct: u16) -> f64 {
    let mut amm_sub = BpfAmm::new_native(
        storage_exp_swap, None, config.initial_x, config.initial_y, "submission".to_string());
    let mut s = [0u8; 4];
    ...
    let mut amm_norm = BpfAmm::new_native(
        normalizer_swap, Some(normalizer_after_swap), norm_x, norm_y, "normalizer".to_string());
...
fn main() {
    ...
    let fee_options: Vec<u16> = vec![30, 40, 50, 55, 60, 65, 70, 80, 100, 120, 150, 200];
    let alpha = 30u16; // fixed α=0.30
    ...
    println!("=== Oracle: Exp(0.30) with per-bucket optimal fee ===");
    let mut oracle_total = 0.0f64;
    let mut oracle_count = 0usize;
    
    for (lo, hi) in &bucket_ranges {
        let idx: Vec<usize> = configs.iter().enumerate()
        ...
        for &fee in &fee_options {
            let total: f64 = idx.par_iter().map(|&i| run_sim(&configs[i], fee, alpha)).sum();
            if total > best_total { best_total = total; best_fee = fee; }
        }
        oracle_total += best_total;
        oracle_count += idx.len();
        println!("  σ [{:.3}%, {:.3}%): {} sims, best_fee={}bp, avg_edge={:.2}",
            lo*100.0, hi*100.0, idx.len(), best_fee, best_total / idx.len() as f64);
    }
    
    println!("\nExp Oracle avg edge: {:.2}", oracle_total / oracle_count as f64);
    
    // Also do per-sim oracle on first 200 sims for more precision
    println!("\n=== Per-sim oracle (200 sims) ===");
    let per_sim_total: f64 = (0..200usize).into_par_iter().map(|i| {
        let mut best = f64::NEG_INFINITY;
        for &fee in &fee_options {
            let e = run_sim(&configs[i], fee, alpha);
            if e > best { best = e; }
        }
        best
    }).sum();
    println!("Per-sim oracle avg edge: {:.2}", per_sim_total / 200.0);
